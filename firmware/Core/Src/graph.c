...
void Process_Graph(uint16_t raw_value) {
    ...
    if (graph_count >= downsample_rate) {
        ...
        // 3. Prepare Buffer (Same as before)
        ...
        if (xSemaphoreTake(lcdSpiSemaphore, portMAX_DELAY) == pdTRUE) {
           ...
           // A. Setup Window (HAL is safe here)
           ILI_Write(0x2A, 1);
           ...
           // We do this to ensure smooth transition to DMA without toggling CS
